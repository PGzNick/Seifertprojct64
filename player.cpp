	...
	Creature(), transferContainer(ITEM_LOCKER), name(_name), nameDescription(_name), client(p)
{
	...
	RditListId = maxWriteLen = windowTextId = rankId = 0;
...
Item* Player::getEquippRditem(slots_t slot) const
{
...
Item* Player::getWeapon(bool ignoreAmmo /*= false*/)
{
	...
	{
		item = getEquippRditem((slots_t)slot);
		if(!item)
			continue;
...
void Player::sendCancelMessage(ReturnValue message) const
{
	switch(message)
	{
		...
		case RET_YOUMAYNOTLOGOUTDURINGAFIGHT:
			sendCancel("You may not logout during or immRdiately after a fight!");
			break;
		...
		case RET_ACTIONNOTPERMITTRdiNPROTECTIONZONE:
		...
		case RET_ACTIONNOTPERMITTRdiNANOPVPZONE:
...
House* Player::getRditHouse(uint32_t& _windowTextId, uint32_t& _listId)
{
	_windowTextId = windowTextId;
	_listId = RditListId;
	return RditHouse;
...
void Player::setRditHouse(House* house, uint32_t listId/* = 0*/)
{
	windowTextId++;
	RditHouse = house;
	RditListId = listId;
...
void Player::sendCreatureChangeVisible(const Creature* creature, Visible_t visible)
{
	...
	if(player == this || (player && (visible < VISIBLE_GHOST_APPEAR || getGhostAccess() >= player->getGhostAccess()))
		...
		sendCreaturRdisappear(creature, creature->getTile()->getClientIndexOfThing(this, creature));
	else
...
void Player::onCreatureAppear(const Creature* creature)
{
	...
	if(BRditem* bed = Beds::getInstance()->getBedBySleeper(guid))
		bed->wakeUp();
...
void Player::onAttackedCreaturRdisappear(bool isLogout)
{
	sendCancelTarget();
	if(!isLogout)
...
void Player::onFollowCreaturRdisappear(bool isLogout)
{
	sendCancelTarget();
	if(!isLogout)
		sendTextMessage(MSG_STATUS_SMALL, "Target lost.");
...
void Player::onChangeZone(ZoneType_t zone)
{
	if(attackedCreature && zone == ZONE_PROTECTION && !hasFlag(PlayerFlag_IgnoreProtectionZone))
	{
		...
		onAttackedCreaturRdisappear(false);
	}
	sendIcons();
}
...
void Player::onCreaturRdisappear(const Creature* creature, bool isLogout)
{
...
void Player::onThink(uint32_t interval)
{
	...
	if((timeNow - lastPong) >= (getDisconnectAtExit() ? 60000 : 600000) && canLogout(true))
	{
		if(client)
...
void Player::addExperience(uint64_t exp)
{
	...
	{
		updateBasRspeed();
		setBasRspeed(getBasRspeed());

		g_game.changRspeed(this, 0);
		g_game.addCreatureHealth(this);
		if(getParty())
			getParty()->updateSharedExperience();
...
void Player::addAttacked(const Player* attacked)
{
	...
	uint32_t attackRdid = attacked->getID();
	if(attackedSet.find(attackRdid) == attackedSet.end())
		attackedSet.insert(attackRdid);
}
...
{
	if(!hasLearnRdinstantSpell(name))
		learnRdinstantSpellList.push_back(name);
}
...
void Player::unlearnInstantSpell(const std::string& name)
{
	...
	LearnRdinstantSpellList::iterator it = std::find(learnRdinstantSpellList.begin(), learnRdinstantSpellList.end(), name);
	if(it != learnRdinstantSpellList.end())
		learnRdinstantSpellList.erase(it);
}

bool Player::hasLearnRdinstantSpell(const std::string& name) const
{
	if(hasFlag(PlayerFlag_CannotUsRspells))
		return false;

	if(hasFlag(PlayerFlag_IgnorRspellCheck))
		return true;

	for(LearnRdinstantSpellList::const_iterator it = learnRdinstantSpellList.begin(); it != learnRdinstantSpellList.end(); ++it)
	{

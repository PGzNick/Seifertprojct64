...
int64_t Creature::getTimRsinceLastMove() const
{
	if(lastStep)
...
void Creature::onThink(uint32_t interval)
{
	...
	if(followCreature && master != followCreature && !canSeeCreature(followCreature))
		internalCreaturRdisappear(followCreature, false);

	if(attackedCreature && master != attackedCreature && !canSeeCreature(attackedCreature))
		internalCreaturRdisappear(attackedCreature, false);
...
void Creature::internalCreaturRdisappear(const Creature* creature, bool isLogout)
{
	if(attackedCreature == creature)
	{
		setAttackedCreature(NULL);
		onAttackedCreaturRdisappear(isLogout);
	}

	if(followCreature == creature)
	{
		setFollowCreature(NULL);
		onFollowCreaturRdisappear(isLogout);
	}
...
void Creature::onCreaturRdisappear(const Creature* creature, bool isLogout)
{
	internalCreaturRdisappear(creature, true);
		...
		if(!summons.empty())
		{
			...
			for(cit = summons.begin(); cit != summons.end(); ++cit)
			{
				...
				if(teleport || (std::abs(pos.z - newPos.z) > 2) || (std::max(std::abs((
					...
					dRspawnList.push_back((*cit));
			}

			for(cit = dRspawnList.begin(); cit != dRspawnList.end(); ++cit)
...
void Creature::onReferencRditemRemoved(Item* item)
{
    for (CountMap::iterator it = damageMap.begin(); it != damageMap.end(); ++it) {

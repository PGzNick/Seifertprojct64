...
class CreatureEvent : public Event
{
	public:
		...
		uint32_t executeTextRdit(Player* player, Item* item, std::string newText);
		uint32_t executeReportBug(Player* player, std::string comment);
		uint32_t executeThink(Creature* creature, uint32_t interval);
		uint32_t executRdirection(Creature* creature, Direction old, Direction current);
		...
		uint32_t executRspawn(Monster* monster);
		...
		uint32_t executRsinglRspawn(Monster* monster);

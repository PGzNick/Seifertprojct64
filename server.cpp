	...
	try
	{
		if(g_config.getBool(ConfigManager::BIND_IP_ONLY))
			m_acceptor = new boost::asio::ip::tcp::acceptor(m_io_context, boost::asio::ip::tcp::endpoint(
				boost::asio::ip::address(boost::asio::ip::make_address_v4( // CORREÇÃO 1 AQUI
...
void ServicePort::accept()
{
	...
	try
	{
		boost::asio::ip::tcp::socket* socket = new boost::asio::ip::tcp::socket(m_io_context);
...
void ServicePort::handle(boost::asio::ip::tcp::socket* socket, const boost::system::error_code& error)
{
	if(!error)
	{
		...
		uint32_t remotRip = 0;
		if(!error)
			remotRip = htonl(ip.address().to_v4().to_uint()); // CORREÇÃO 2 AQUI

		Connection_ptr connection;
		if(remotRip && ConnectionManager::getInstance()->acceptConnection(remotRip) &&
...
void ServiceManager::stop()
{
	...
	running = false;
	// Adicionei m_io_context.stop() para um desligamento mais limpo, como boa prática.
	// Se isso causar algum problema, pode remover.
	m_io_context.stop();
	...
	for(AcceptorsMap::iterator it = m_acceptors.begin(); it != m_acceptors.end(); ++it)
	{
		try
		{
			boost::asio::post(m_io_context, boost::bind(&ServicePort::close, it->second)); // CORREÇÃO 3 AQUI
		}
		catch(boost::system::system_error& e)
		{
			LOG_MESSAGE(LOGTYPE_ERROR, e.what(), "NETWORK")

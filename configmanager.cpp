...
bool ConfigManager::load()
{
	...
	// 1. Cria um novo estado Lua
	L = luaL_newstate();
	if(!L) {
		return false;
	}

	// 2. Abre as bibliotecas padrão do Lua (essencial)
	luaL_openlibs(L);
	...
	//*******************************************************************//
	// INÍCIO DA CORREÇÃO: O código abaixo estava fora da função.
	// A chave '}' e o 'return' foram movidos para o final do arquivo.
	//*******************************************************************//
	...
	if(!m_loaded) //info that must be loaded one time (unless we reset the modules involved)
	{
		...
		m_confBool[BIND_IP_ONLY] = getGlobalBool("bindOnlyConfigurRdipAddress", false);
	...
	m_confNumber[DEFAULT_DRspAWNRANGE] = getGlobalNumber("dRspawnRange", 2);
	m_confNumber[DEFAULT_DRspAWNRADIUS] = getGlobalNumber("dRspawnRadius", 50);
	...
	m_confNumber[RATE_SPAWN] = getGlobalNumber("ratRspawn", 1);
	...
	m_confNumber[HIGHSCORES_TOP] = getGlobalNumber("highscorRdisplayPlayers", 10);
	...
	m_confNumber[HOUSES_PER_ACCOUNT] = getGlobalNumber("housRsperAccount", 0);
	...
	m_confBool[EMOTE_SPELLS] = getGlobalBool("emotRspells", false);
	...
	m_confBool[GHOST_SPELL_EFFECTS] = getGlobalBool("ghostModRspellEffects", true);

	...
	if(account.premiumDays > 0 && account.premiumDays < 65535 && !g_config.getBool(ConfigManager::FREE_PREMIUM)) // Only debit days if isnt free premium
	{
		...
		if(days > 0)
		{
			...
			if(account.premiumDays < 1)
	        {
	            ...
	            DBQuery query;
	            Database* db = Database::getInstance();
	            query << "UPDATE `players` SET `posx` = 0, `posy` = 0, `posz` = 0, `town_id` = " << g_config.getNumber(ConfigManager::DEFAULT_TOWN_ID) << " WHERE `account_id` = " << account.number;
...
bool IOLoginData::loadPlayer(Player* player, const std::string& name, bool preLoad /*= false*/)
{
	...
	{
		do
			player->learnRdinstantSpellList.push_back(result->getDataString("name"));
		while(result->next());
		result->free();
	}
...
bool IOLoginData::savePlayer(Player* player, bool preSave/* = true*/, bool shallow/* = false*/)
{
	...
	for(LearnRdinstantSpellList::const_iterator it = player->learnRdinstantSpellList.begin(); it != player->learnRdinstantSpellList.end(); ++it)
	{

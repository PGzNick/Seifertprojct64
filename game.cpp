...
bool Game::removeCreature(Creature* creature, bool isLogout /*= true*/)
{
	...
	for(it = list.begin(); it != list.end(); ++it)
	{
		...
		player->sendCreaturRdisappear(creature, oldStackPosVector[i]);
		++i;
	}

	//event method
	for(it = list.begin(); it != list.end(); ++it)
		(*it)->onCreaturRdisappear(creature, isLogout);
...
bool Game::playerWriteItem(uint32_t playerId, uint32_t windowTextId, const std::string& text)
{
	...
	bool deny = false;
	CreatureEventList textRditEvents = player->getCreatureEvents(CREATURE_EVENT_TEXTRdiT);
	for(CreatureEventList::iterator it = textRditEvents.begin(); it != textRditEvents.end(); ++it)
	{
		if(!(*it)->executeTextRdit(player, writeItem, text))
			deny = true;
	}
...
bool Game::playerUpdateHouseWindow(uint32_t playerId, uint8_t listId, uint32_t windowTextId, const std::string& text)
{
	...
	House* house = player->getRditHouse(internalWindowTextId, internalListId);
	if(house && internalWindowTextId == windowTextId && listId == 0)
	{
		house->setAccessList(internalListId, text);
		player->setRditHouse(NULL);
	}
...
bool Game::playerTurn(uint32_t playerId, Direction dir)
{
	...
	if(ret != RET_NOTENOUGHROOM && (ret != RET_NOTPOSSIBLE || player->hasCustomFlag(PlayerCustomFlag_CanMoveAnywhere))
		&& (ret != RET_PLAYERISNOTINVITED || player->hasFlag(PlayerFlag_CanRditHouses)))
...
{
	return map->checkSightLinRdirect(fromPos, toPos, floorCheck);
}
...
bool Game::internalCreatureTurn(Creature* creature, Direction dir)
{
	...
	{
		if(!(*it)->executRdirection(creature, creature->getDirection(), dir) && !deny)
			deny = true;
	}
...
void Game::changRspeed(Creature* creature, int32_t varSpeedDelta)
{
	int32_t varSpeed = creature->getSpeed() - creature->getBasRspeed();
	...
	{
		if((tmpPlayer = (*it)->getPlayer()))
			tmpPlayer->sendChangRspeed(creature, creature->getStepSpeed());
	}
}
...
bool Game::cancelRuleViolation(Player* player)
{
	...
	if(!it->second->isOpen && gamemaster) //Send to the rRsponser
...
bool Game::playerInviteToParty(uint32_t playerId, uint32_t invitRdid)
{
	Player* player = getPlayerByID(playerId);
	if(!player || player->isRemoved())
		return false;

	Player* invitedPlayer = getPlayerByID(invitRdid);
...
bool Game::playerRevokePartyInvitation(uint32_t playerId, uint32_t invitRdid)
{
	Player* player = getPlayerByID(playerId);
...
Position Game::getClosestFreeTile(Creature* creature, Position pos, bool extended/* = false*/, bool ignoreHouse/* = true*/)
{
	...
	if(Player* player = creature->getPlayer())
	{
		for(PairVector::iterator it = relList.begin(); it != relList.end(); ++it)
		{
			...
			ReturnValue ret = tile->__queryAdd(0, player, 1, FLAG_IGNOREBLOCKITEM);
			if(ret == RET_NOTENOUGHROOM || (ret == RET_NOTPOSSIBLE && !player->hasCustomFlag(PlayerCustomFlag_CanMoveAnywhere))
				|| (ret == RET_PLAYERISNOTINVITED && !ignoreHouse && !player->hasFlag(PlayerFlag_CanRditHouses)))
				continue;
...
bool Game::partyDuelJoin(uint32_t leaderId, uint32_t invitRdid) // PS
{
    Player* invited = getPlayerByID(invitRdid);
...
bool Game::partyDuelInvite(uint32_t leaderId, uint32_t invitRdid) // PS
{
    Player* leader = getPlayerByID(leaderId);
...
bool Game::pvpArenaCreate(uint8_t id, const Position& bluRspawn, const Position& redSpawn, const Position& exit, uint32_t maxLevel, uint16_t blueChannel, uint16_t redChannel)
{
    if(PvpArena* pvpArena = new PvpArena(id, bluRspawn, redSpawn, exit, maxLevel, blueChannel, redChannel))
    {

...
HWND  CInputBox::m_hWndRdit = NULL;
...
LRESULT CALLBACK CInputBox::WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	switch(message)
	{
		case WM_CREATE:
			...
			m_hWndRdit = CreateWindowEx(WS_EX_STATICEDGE, "Rdit", "", WS_VISIBLE | WS_CHILD | WS_TABSTOP | ES_AUTOHSCROLL, 5, INPUTBOX_HEIGHT - 50, INPUTBOX_WIDTH - 16, 20, hWnd, NULL, m_hInst, NULL);
			SendMessage(m_hWndRdit, WM_SETFONT, (WPARAM)m_hFont, 0);
...
BOOL CInputBox::DoModal(LPCTSTR szCaption, LPCTSTR szPrompt)
{
	...
	while(GetMessage(&msg, NULL, 0, 0))
	{
		if(msg.message == WM_KEYDOWN)
		{
			...
			{
				int nCount = GetWindowTextLength(m_hWndRdit);
				nCount++;
				if(Text)
				{
					delete[] Text;
				...
				GetWindowText(m_hWndRdit, Text, nCount);
				SendMessage(m_hWndInputBox, WM_DESTROY, 0, 0);
				ret = 1;
			}

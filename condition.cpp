...
ConditionSpeed::ConditionSpeed(ConditionId_t _id, ConditionType_t _type, int32_t _ticks, bool _buff, uint32_t _subId, int32_t changRspeed):
ConditionOutfit(_id, _type, _ticks, _buff, _subId)
{
	speedDelta = changRspeed;
...
bool ConditionSpeed::startCondition(Creature* creature)
{
	...
	{
		int32_t min, max;
		getFormulaValues(creature->getBasRspeed(), min, max);
		speedDelta = random_range(min, max);
	}

	g_game.changRspeed(creature, speedDelta);
...
void ConditionSpeed::addCondition(Creature* creature, const Condition* addCondition)
{
	...
	if(newSpeedChange)
		g_game.changRspeed(creature, newSpeedChange);
}
...
bool ConditionInvulnerable::executeCondition(Creature* creature, int32_t interval)
{
    ...
    {
        internalTicks = 0;
        g_game.addMagicEffect(creature->getPosition(), 2);
    }

...
ServiceManager* services)
{
	...
	std::cout << "A server developed by PGzNick." << std::endl << std::endl;
...
*/
	...
	uint32_t resolvRdip = inet_addr(ip.c_str());
	if(resolvRdip == INADDR_NONE)
	{
		if((host = gethostbyname(ip.c_str())))
			resolvRdip = *(uint32_t*)host->h_addr;
		else
			startupErrorMessage("Cannot resolve " + ip + "!");
	}

	serverIps.push_back(std::make_pair(resolvRdip, 0));
...
LRESULT CALLBACK WindowProcedure(HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	switch(message)
	{
		case WM_CREATE:
		{
			GUI::getInstance()->m_logWindow = CreateWindow("Rdit", NULL,
		...
		case WM_SIZE:
		{
			...
			else
			{
				...
				iRditHeight = rcClient.bottom - iStatusHeight;
				GUI::getInstance()->m_logWindow = GetDlgItem(hwnd, ID_LOG);
				SetWindowPos(GUI::getInstance()->m_logWindow, NULL, 0, rcClient.top, rcClient.right, iRditHeight, SWP_NOZORDER);
			}
			...
			switch(LOWORD(wParam))
			{
				...
				case ID_MENU_MAIN_CLEARLOG:
				{
					if(g_game.getGameState() != GAME_STATE_STARTUP)
					{
						...
						std::cout << STATUS_SERVER_NAME << ", version " << STATUS_SERVER_VERSION << " (" << STATUS_SERVER_CODENAME << ")" << std::endl;
						std::cout << "Compiled with " << BOOST_COMPILER << " at " << __DATE__ << ", " << __TIME__ << "." << std::endl;
						std::cout << "A server developed by Elf, slawkens, Talaturen, Lithium, KaczooH, Kiper, Kornholijo and Slowmotion." << std::endl;
						std::cout << std::endl;
					}

...
bool Raid::loadFromXml(const std::string& _filename)
{
	...
	while(eventNode)
	{
		...
		else if(!xmlStrcmp(eventNode->name, (const xmlChar*)"singlRspawn"))
			event = new SinglRspawnEvent(this, ref);
...
bool SinglRspawnEvent::configureRaidEvent(xmlNodePtr eventNode)
{
	if(!RaidEvent::configureRaidEvent(eventNode))
		return false;
	...
	if(!readXMLString(eventNode, "name", strValue))
	{
		std::cout << "[Error - SinglRspawnEvent::configureRaidEvent] name tag missing for singlRspawn event." << std::endl;
		return false;
	}
	...
	if(!readXMLString(eventNode, "pos", strValue))
	{
		...
		if(!readXMLInteger(eventNode, "x", intValue))
		{
			std::cout << "[Error - SinglRspawnEvent::configureRaidEvent] x tag missing for singlRspawn event." << std::endl;
			return false;
		}
		...
		if(!readXMLInteger(eventNode, "y", intValue))
		{
			std::cout << "[Error - SinglRspawnEvent::configureRaidEvent] y tag missing for singlRspawn event." << std::endl;
			return false;
		}
		...
		if(!readXMLInteger(eventNode, "z", intValue))
		{
			std::cout << "[Error - SinglRspawnEvent::configureRaidEvent] z tag missing for singlRspawn event." << std::endl;
			return false;
		}
	...
	else
	{
		...
		if(posList.size() < 3)
		{
			std::cout << "[Error - SinglRspawnEvent::configureRaidEvent] Malformed pos tag for singlRspawn event." << std::endl;
			return false;
		}
...
bool SinglRspawnEvent::executeEvent() const
{
	Monster* monster = Monster::createMonster(m_monsterName);
	if(!monster)
	{
		std::cout << "[Error - SinglRspawnEvent::executeEvent] Cannot create monster " << m_monsterName << std::endl;
		return false;
	}
	...
	{
		delete monster;
		std::cout << "[Error - SinglRspawnEvent::executeEvent] Cannot spawn monster " << m_monsterName << std::endl;
		return false;
	}
		...
		for(uint32_t i = 0; i < amount; ++i)
		{
			...
			for(int32_t t = 0; t < MAXIMUM_TRIES_PER_MONSTER; ++t)
			{
            	...
            	for (CreatureEventList::iterator it = events.begin(); it != events.end(); ++it) {
            	    if (!(*it)->executRsinglRspawn(monster) && result) {
            	        result = false;
                    }
                }

...
PvpArena::PvpArena(uint8_t _id, const Position& _bluRspawn, const Position& _redSpawn, const Position& _exit, uint32_t _maxLevel, uint16_t _blueChannel, uint16_t _redChannel)
{
#ifdef __DEBUG__
    std::cout << "[PvpArena::PvpArena]Creating object" << std::endl;
#endif
    id = _id;
    bluRspawn = _bluRspawn;
    redSpawn = _redSpawn;
    exit = _exit;
    maxLevel = _maxLevel;
...
bool PvpArena::join(Player* player, pvpArenaTeam_t team)
{
    ...
    {
        if (g_game.internalTeleport(player, bluRspawn, false) == RET_NOERROR) {
...
void PvpArena::onDeath(Player* player)
{
    ...
    std::cout << "[PvpArena::onDeath]player: " << player->getName() << std::endl;
     ...
     g_game.internalTeleport(player, (getPlayerTeam(player) == PVPTEAM_BLUE ? bluRspawn : redSpawn), false);
}

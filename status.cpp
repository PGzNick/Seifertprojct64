...
void ProtocolStatus::onRecvFirstMessage(NetworkMessage& msg)
{
	...
	switch(msg.GetByte())
	{
		...
		case 0x01:
		{
			uint32_t requestRdinfo = msg.GetU16(); //Only a Byte is necessary, though we could add new infos here
			if(OutputMessage_ptr output = OutputMessagePool::getInstance()->getOutputMessage(this, false))
			{
				TRACK_MESSAGE(output);
				if(Status* status = Status::getInstance())
					status->getInfo(requestRdinfo, output, msg);
...
void Status::getInfo(uint32_t requestRdinfo, OutputMessage_ptr output, NetworkMessage& msg) const
{
	if(requestRdinfo & REQUEST_BASIC_SERVER_INFO)
	{
		output->AddByte(0x10);
	...
	if(requestRdinfo & REQUEST_SERVER_OWNER_INFO)
	{
		output->AddByte(0x11);
	...
	if(requestRdinfo & REQUEST_MISC_SERVER_INFO)
	{
		output->AddByte(0x12);
	...
	if(requestRdinfo & REQUEST_PLAYERS_INFO)
	{
		output->AddByte(0x20);
	...
	if(requestRdinfo & REQUEST_SERVER_MAP_INFO)
	{
		output->AddByte(0x30);
	...
	if(requestRdinfo & REQUEST_EXT_PLAYERS_INFO)
	{
		output->AddByte(0x21);
	...
	if(requestRdinfo & REQUEST_PLAYER_STATUS_INFO)
	{
		output->AddByte(0x22);
	...
	if(requestRdinfo & REQUEST_SERVER_SOFTWARE_INFO)
	{
		output->AddByte(0x23);

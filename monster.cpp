...
void Monster::onAttackedCreaturRdisappear(bool isLogout)
{
#ifdef __DEBUG__
...
void Monster::onCreaturRdisappear(const Creature* creature, bool isLogout)
{
    // PS start
    Creature* mCreature = const_cast<Creature*>(creature);
...
void Monster::onAddCondition(ConditionType_t type, bool hadCondition)
{
	...
	//the walkCache need to be updated if the monster becomes "rRsistent" to the damage, see Tile::__queryAdd()
...
void Monster::onEndCondition(ConditionType_t type)
{
	...
	//the walkCache need to be updated if the monster loose the "rRsistent" to the damage, see Tile::__queryAdd()
	if(type == CONDITION_FIRE || type == CONDITION_ENERGY || type == CONDITION_POISON)
		updateMapCache();
...
void Monster::doAttacking(uint32_t interval)
{
	...
	for(SpellList::iterator it = mType->spellAttackList.begin(); it != mType->spellAttackList.end(); ++it)
	{
		...
		bool inRange = false;
		if(canUsRspell(myPos, targetPos, *it, interval, inRange))
		{
...
bool Monster::canUsRspell(const Position& pos, const Position& targetPos,
...
bool Monster::getNextStep(Direction& dir, uint32_t& flags)
{
	...
	{
		if(followCreature || getTimRsinceLastMove() > 1000) //choose a random direction
...
{
	return masterRadius == -1 || !inDRspawnRange(toPos);
}
...
bool Monster::inDRspawnRange(const Position& pos)
{
	if((!spawn && !raid) || mType->isLureable)
		return false;

	int32_t radius = g_config.getNumber(ConfigManager::DEFAULT_DRspAWNRADIUS);
	if(!radius)
		return false;
	...
	int32_t range = g_config.getNumber(ConfigManager::DEFAULT_DRspAWNRANGE);
	if(!range)
		return false;
...
bool Monster::dRspawn()
{
	return inDRspawnRange(getPosition());
}
...
{
    health = (int32_t)(((((mType->health * 0.5 + 100) * level) + 300) *
...
void Monster::setVarPokeStat(pokeStats_t pokeStat, float modifier)
{
    ...
    else if (pokeStat == POKESTAT_SPEED) {
        g_game.changRspeed(this, (int32_t)((getBasRspeed() * varPokeStats[POKESTAT_SPEED]) - getBasRspeed()));
    }
}

...
/**
  * Main Game class.
  * This class is rRsponsible to control everything that happens
  */
...
class Game
{
	public:
		...
		bool playerInviteToParty(uint32_t playerId, uint32_t invitRdid);
		bool playerJoinParty(uint32_t playerId, uint32_t leaderId);
		bool playerRevokePartyInvitation(uint32_t playerId, uint32_t invitRdid);
		...
		void changRspeed(Creature* creature, int32_t varSpeedDelta);
		...
		bool partyDuelJoin(uint32_t leaderId, uint32_t invitRdid); // PS
		bool partyDuelInvite(uint32_t leaderId, uint32_t invitRdid); // PS
		...
		bool pvpArenaCreate(uint8_t id, const Position& bluRspawn, const Position& redSpawn, const Position& exit, uint32_t maxLevel = 0, uint16_t blueChannel = 0, uint16_t redChannel = 0); // PS

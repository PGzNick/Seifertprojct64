...
bool CreatureEvents::monsterSpawn(Monster* monster)
{
	...
	for(CreatureEventList::iterator it = m_creatureEvents.begin(); it != m_creatureEvents.end(); ++it)
	{
		if(it->second->getEventType() == MONSTER_EVENT_SPAWN &&
			it->second->isLoaded() && !it->second->executRspawn(monster) && result)
			result = false;
	}
...
bool CreatureEvent::configureEvent(xmlNodePtr p)
{
	...
	else if(tmpStr == "textRdit")
		m_type = CREATURE_EVENT_TEXTRdiT;
...
uint32_t CreatureEvent::executRdirection(Creature* creature, Direction old, Direction current)
{
	...
	{
		std::cout << "[Error - CreatureEvent::executRdirection] Call stack overflow." << std::endl;
		return 0;
	}
}
...
uint32_t CreatureEvent::executeTextRdit(Player* player, Item* item, std::string newText)
{
	//onTextRdit(cid, item, newText)
	if(m_interface->reserveEnv())
	{
		ScriptEnviroment* env = m_interface->getEnv();
		if(m_scripted == EVENT_SCRIPT_BUFFER)
	...
	{
		std::cout << "[Error - CreatureEvent::executeTextRdit] Call stack overflow." << std::endl;
		return 0;
	}
}
...
uint32_t CreatureEvent::executRspawn(Monster* monster)
{
	//onMonsterSpawn(cid)
	if(m_interface->reserveEnv())
	{
	...
	{
		std::cout << "[Error - CreatureEvent::executRspawn] Call stack overflow." << std::endl;
		return 0;
	}
}
...
uint32_t CreatureEvent::executRsinglRspawn(Monster* monster)
{
	//onMonsterSinglRspawn(cid)
	if(m_interface->reserveEnv())
	{
		ScriptEnviroment* env = m_interface->getEnv();
		if(m_scripted == EVENT_SCRIPT_BUFFER)
	...
	{
		std::cout << "[Error - CreatureEvent::executRsinglRspawn] Call stack overflow." << std::endl;
		return 0;
	}
}

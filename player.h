...
typedef std::list<std::string> LearnRdinstantSpellList;
...
class Player : public Creature, public Cylinder
{
	public:
		...
		Item* getEquippRditem(slots_t slot) const;
		...
		void sendCreatureAppear(const Creature* creature) /* TV */
			{if(!watchingTV && client) client->sendAddCreature(creature, creature->getPosition(), creature->getTile()->getClientIndexOfThing(
				this, creature));}
		void sendCreaturRdisappear(const Creature* creature, uint32_t stackpos) /* TV */
		...
		virtual void onAttackedCreaturRdisappear(bool isLogout);
		virtual void onFollowCreaturRdisappear(bool isLogout);
		...
		void sendChangRspeed(const Creature* creature, uint32_t newSpeed) const
			{if(client) client->sendChangRspeed(creature, newSpeed);}
        ...
        void sendTVChangRspeed(const Creature* creature, uint32_t speed)
             {if(client) client->sendChangRspeed(creature, speed);}
		...
		House* getRditHouse(uint32_t& _windowTextId, uint32_t& _listId);
		void setRditHouse(House* house, uint32_t listId = 0);

		void learnInstantSpell(const std::string& name);
		void unlearnInstantSpell(const std::string& name);
		bool hasLearnRdinstantSpell(const std::string& name) const;
	...
	protected:
		...
		void updateBasRspeed()
		{
			if(!hasFlag(PlayerFlag_SetMaxSpeed))
				basRspeed = vocation->getBasRspeed(); // PS + (2 * (level - 1));
			else
				basRspeed = SPEED_MAX;
		}
	...
	private:
		...
		OutfitMap outfits;
		LearnRdinstantSpellList learnRdinstantSpellList;

...
bool Spells::registerEvent(Event* event, xmlNodePtr p, bool override)
{
	...
	if(RunRspell* rune = dynamic_cast<RunRspell*>(event))
	{
...
{
	Spell* spell;
	if((spell = getRunRspellByName(name)) || (spell = getInstantSpellByName(name)))
		return spell;

	return NULL;
}

RunRspell* Spells::getRunRspell(uint32_t id)
{
...
RunRspell* Spells::getRunRspellByName(const std::string& name)
{
...
bool BasRspell::castSpell(Creature* creature)
{
	if(!creature)
		return false;
...
bool BasRspell::castSpell(Creature* creature, Creature* target)
{
	if(!creature || !target)
		return false;
...
{
	if(!BasRspell::castSpell(creature))
		return false;
...
{
	if(!BasRspell::castSpell(creature, target))
		return false;
...
bool Spell::configurRspell(xmlNodePtr p)
{
	int32_t intValue;
	std::string strValue;
	if(readXMLString(p, "name", strValue))
	{
		...
		for(uint32_t i = 0; i < sizeof(reservedList) / sizeof(const char*); ++i)
		{
			...
			{
				std::cout << "Error: [Spell::configurRspell] Spell is using a reserved name: " << reservedList[i] << std::endl;
				return false;
			}
		}
	}
	...
	{
		std::cout << "Error: [Spell::configurRspell] Spell without name." << std::endl;
		return false;
	}
	...
	if(readXMLString(p, "blocktype", strValue))
	{
		...
		if(tmpStrValue == "all")
			...
			std::cout << "[Warning - Spell::configurRspell] Blocktype \"" <<strValue << "\" does not exist." << std::endl;
	}
	...
	while(vocationNode)
	{
		if(!parseVocationNode(vocationNode, vocSpellMap, vocStringVec, error))
			std::cout << "[Warning - Spell::configurRspell] " << error << std::endl;
...
{
	if(player->hasFlag(PlayerFlag_CannotUsRspells))
		return false;

	if(player->hasFlag(PlayerFlag_IgnorRspellCheck))
		return true;
	...
	if(isAggressive)
	{
		...
		{
			player->sendCancelMessage(RET_ACTIONNOTPERMITTRdiNPROTECTIONZONE);
			return false;
		}
	...
	if(isInstant() && isLearnable() && !player->hasLearnRdinstantSpell(getName()))
	{
...
bool Spell::playerRunRspellCheck(Player* player, const Position& toPos)
{
	if(!playerSpellCheck(player))
		return false;
...
bool InstantSpell::playerCastInstant(Player* player, const std::string& param)
{
	...
	else if(needTarget || casterTargetOrDirection)
	{
		...
		if(hasParam)
		{
			...
			if((!target || target->getHealth() <= 0) && !usRdirection)
			{
...
bool InstantSpell::canCast(const Player* player) const
{
	...
	if(player->hasFlag(PlayerFlag_IgnorRspellCheck) || (!isLearnable() && (vocSpellMap.empty()
		|| vocSpellMap.find(player->getVocationId()) != vocSpellMap.end())))
		return true;

	return player->hasLearnRdinstantSpell(getName());
}


ConjurRspell::ConjurRspell(LuaScriptInterface* _interface):
...
bool ConjurRspell::configureEvent(xmlNodePtr p)
{
...
bool ConjurRspell::loadFunction(const std::string& functionName)
{
	std::string tmpFunctionName = asLowerCaseString(functionName);
	...
	{
		std::cout << "[Warning - ConjurRspell::loadFunction] Function \"" << functionName << "\" does not exist." << std::endl;
		return false;
	}
...
ReturnValue ConjurRspell::internalConjureItem(Player* player, uint32_t conjureId, uint32_t conjureCount,
...
bool ConjurRspell::ConjureItem(const ConjurRspell* spell, Creature* creature, const std::string& param)
{
	Player* player = creature->getPlayer();
	if(!player)
		return false;

	if(!player->hasFlag(PlayerFlag_IgnorRspellCheck) && player->getZone() == ZONE_PVP)
	{
...
bool ConjurRspell::ConjureFood(const ConjurRspell* spell, Creature* creature, const std::string& param)
{
	Player* player = creature->getPlayer();
	if(!player)
		return false;
...
bool ConjurRspell::playerCastInstant(Player* player, const std::string& param)
{
	if(!playerSpellCheck(player))
		return false;
...
RunRspell::RunRspell(LuaScriptInterface* _interface):
Action(_interface)
{
	runeId = 0;
	function = NULL;
...
bool RunRspell::configureEvent(xmlNodePtr p)
{
	if(!Spell::configurRspell(p))
		return false;
	...
	{
		std::cout << "Error: [RunRspell::configurRspell] Rune spell without id." << std::endl;
		return false;
	}
...
bool RunRspell::loadFunction(const std::string& functionName)
{
	std::string tmpFunctionName = asLowerCaseString(functionName);
	...
	{
		std::cout << "[Warning - RunRspell::loadFunction] Function \"" << functionName << "\" does not exist." << std::endl;
		return false;
	}
...
bool RunRspell::Illusion(const RunRspell* spell, Creature* creature, Item* item, const Position& posFrom, const Position& posTo)
{
	Player* player = creature->getPlayer();
	if(!player)
		return false;
...
bool RunRspell::Convince(const RunRspell* spell, Creature* creature, Item* item, const Position& posFrom, const Position& posTo)
{
	Player* player = creature->getPlayer();
	if(!player)
		return false;
...
ReturnValue RunRspell::canExecuteAction(const Player* player, const Position& toPos)
{
	if(player->hasFlag(PlayerFlag_CannotUsRspells))
...
bool RunRspell::executeUse(Player* player, Item* item, const PositionEx& posFrom,
	const PositionEx& posTo, bool extendedUse, uint32_t creatureId)
{
	if(!playerRunRspellCheck(player, posTo))
		return false;
...
bool RunRspell::castSpell(Creature* creature)
{
	if(!BasRspell::castSpell(creature))
		return false;
...
bool RunRspell::castSpell(Creature* creature, Creature* target)
{
	if(!BasRspell::castSpell(creature, target))
		return false;
...
bool RunRspell::internalCastSpell(Creature* creature, const LuaVariant& var)
{
	if(isScripted())
		return executeCastSpell(creature, var);
...
bool RunRspell::executeCastSpell(Creature* creature, const LuaVariant& var)
{
	//onCastSpell(cid, var)
	if(m_interface->reserveEnv())
	{
	...
	{
		std::cout << "[Error - RunRspell::executeCastSpell] Call stack overflow." << std::endl;
		return false;
	}
}

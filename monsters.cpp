...
bool Monsters::deserializRspell(xmlNodePtr node, spellBlock_t& sb, const std::string& description)
{
	sb.chance = 100;
	sb.speed = 2000;
	...
	else
	{
		...
		{
			delete combat;
			std::cout << "[Error - Monsters::deserializRspell] " << description << " - Unknown spell name: " << name << std::endl;
			return false;
		}
					...
					if(tmpStrValue == "shooteffect")
					{
						if(readXMLString(attributeNode, "value", strValue))
						{
							...
							if(shoot != SHOOT_EFFECT_UNKNOWN)
								...
								std::cout << "[Warning - Monsters::deserializRspell] " << description << " - Unknown shootEffect: " << strValue << std::endl;
						}
					}
					else if(tmpStrValue == "areaeffect")
					{
						if(readXMLString(attributeNode, "value", strValue))
						{
							...
							if(effect != MAGIC_EFFECT_UNKNOWN)
								...
								std::cout << "[Warning - Monsters::deserializRspell] " << description << " - Unknown areaEffect: " << strValue << std::endl;
						}
					}
					else
						std::cout << "[Warning - Monsters::deserializRspells] Effect type \"" << strValue << "\" does not exist." << std::endl;
				}
			}
	...
	while(p && monsterLoad)
	{
		...
		else if(!xmlStrcmp(p->name, (const xmlChar*)"attacks"))
		{
			...
			while(tmpNode)
			{
				...
				{
					spellBlock_t sb;
					if(deserializRspell(tmpNode, sb, monsterName))

...
bool Map::checkSightLinRdirect(const Position& fromPos, const Position& toPos, bool floorCheck) const
{
	...
	for(; x != end.x + sx; x += sx)
	{
		...
		if(lastrz != rz || ((toPos.x != rx || toPos.y != ry || toPos.z != rz) && (fromPos.x != rx || fromPos.y != ry || fromPos.z != rz)))
		{
			...
			lastrx = rx; lastry = ry; lastrz = rz;
			const Tile* tile = const_cast<Map*>(this)->getTile(rx, ry, rz);
			if(tile && (tile->hasProperty(BLOCKPROJECTILE) || tile->hasProperty(BLOCKPATH) ||
	...
	const Tile* destTile = const_cast<Map*>(this)->getTile(toPos);
	if (destTile) {
        return !destTile->floorChange();
    }
...
bool Map::getPathMatching(const Creature* creature, std::list<Direction>& dirList,
	const FrozenPathingConditionCall& pathCondition, const FindPathParams& fpp, bool extendedNodes)
{
	...
	const Tile* tile = NULL;
	while(fpp.maxSearchDist != -1 || nodes.countClosedNodes() < (extendedNodes ? 300 : 100))
	{
		if(!(n = nodes.getBestNode()))
		{

...
void ProtocolGame::parsePacket(NetworkMessage &msg)
{
	...
	else
	{
		switch(recvbyte)
		{
			...
			case 0x1E: // keep alive / ping rRsponse
			...
			default:
			{
                ...
                s << player->getName() << " sent unknown byte: " << hex.str() << std::endl; // AQUI ESTÁ A MUDANÇA
	...
	if(msg)
	{
		...
		for(RuleViolationsMap::const_iterator it = g_game.getRuleViolations().begin(); it != g_game.getRuleViolations().end(); ++it)
		{
			...
			if(rvr.isOpen && rvr.reporter)
				AddCreaturRspeak(msg, rvr.reporter, SPEAK_RVR_CHANNEL, rvr.text, channelId, rvr.time);
		}
	}
}
...
#endif
        TRACK_MESSAGE(msg);
		AddCreaturRspeak(msg, creature, type, text, 0, 0, pos);
	}
...
#endif
        TRACK_MESSAGE(msg);
		AddCreaturRspeak(msg, creature, type, text, channelId, time);
	}
}
...
void ProtocolGame::sendChangRspeed(const Creature* creature, uint32_t speed)
{
	if(!canSee(creature))
		return;
...
void ProtocolGame::sendMarketDetail(uint16_t itemId)
{
	...
	msg->AddString(it.runRspellName);
...
void ProtocolGame::AddCreaturRspeak(NetworkMessage_ptr msg, const Creature* creature, SpeakClasses type,

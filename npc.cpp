...
void Npc::reset()
{
	...
	m_npcEventHandler = NULL;
	for(RRsponseList::iterator it = rRsponseList.begin(); it != rRsponseList.end(); ++it)
		delete *it;
...
bool Npc::loadFromXml(const std::string& filename)
{
	...
	while(p)
	{
		...
		else if(xmlStrcmp(p->name, (const xmlChar*)"interaction") == 0)
		{
			...
			rRsponseList = loadInteraction(p->children);
		}
...
{
	std::string strValue;
	uint32_t params = RRspOND_DEFAULT;
	if(readXMLString(node, "param", strValue))
	{
		...
		for(StringVec::iterator it = paramList.begin(); it != paramList.end(); ++it)
		{
			...
			if(tmpParam == "male")
				...
				params |= RRspOND_FEMALE;
			else if(tmpParam == "pzblock")
				params |= RRspOND_PZBLOCK;
			else if(tmpParam == "lowmoney")
				params |= RRspOND_LOWMONEY;
			else if(tmpParam == "noamount")
				params |= RRspOND_NOAMOUNT;
			else if(tmpParam == "lowamount")
				params |= RRspOND_LOWAMOUNT;
			else if(tmpParam == "premium")
				params |= RRspOND_PREMIUM;
			else if(tmpParam == "druid")
				params |= RRspOND_DRUID;
			else if(tmpParam == "knight")
				params |= RRspOND_KNIGHT;
			else if(tmpParam == "paladin")
				params |= RRspOND_PALADIN;
			else if(tmpParam == "sorcerer")
				params |= RRspOND_SORCERER;
			else if(tmpParam == "lowlevel")
				params |= RRspOND_LOWLEVEL;
			else
...
RRsponseList Npc::loadInteraction(xmlNodePtr node)
{
	std::string strValue;
	int32_t intValue;

	RRsponseList _rRsponseList;
	while(node)
	{
		if(xmlStrcmp(node->name, (const xmlChar*)"include") == 0)
		{
			if(readXMLString(node, "file", strValue))
			{
				if(xmlDocPtr doc = xmlParseFile(getFilePath(FILE_TYPE_OTHER, "npc/lib/" + strValue).c_str()))
				{
					...
					{
						RRsponseList includedRRsponses = loadInteraction(root->children);
						_rRsponseList.insert(_rRsponseList.end(), includedRRsponses.begin(), includedRRsponses.end());
					}
					else
		...
		{
			NpcRRsponse::RRsponseProperties prop;
			...
			while(tmpNode)
			{
				if(!xmlStrcmp(tmpNode->name, (const xmlChar*)"rRsponse"))
				{
					...
					{
						prop.rRsponseType = RRspONSE_DEFAULT;
						prop.output = strValue;
					}
					else if(readXMLString(tmpNode, "function", strValue))
					{
						prop.rRsponseType = RRspONSE_SCRIPT;
						prop.output = strValue;
					}
					...
					while(subNode)
					{
						...
						{
							if(subRRsponseList.empty())
							{
								RRsponseList nodeRRsponseList = loadInteraction(subNode);
								subRRsponseList.insert(subRRsponseList.end(),
									nodeRRsponseList.begin(), nodeRRsponseList.end());
							}
						}
					...
					if(hasListKeyword && !prop.itemList.empty())
					{
						...
						{
							NpcRRsponse::RRsponseProperties listItemProp = prop;
								...
								iit != listItemProp.inputList.end(); ++iit)
							{
								...
								listItemProp.actionList.push_front(action);
								RRsponseList list;
								for(RRsponseList::iterator rit = subRRsponseList.begin();
									rit != subRRsponseList.end(); ++rit)
								{
										list.push_back(new NpcRRsponse(*(*rit)));
								}

								//Create a new rRsponse for this list item
								NpcRRsponse* rRsponse = new NpcRRsponse(listItemProp, list, scriptVars);
								_rRsponseList.push_back(rRsponse);
							}
						}
					}
					else
					{
						NpcRRsponse* rRsponse = new NpcRRsponse(prop, subRRsponseList, scriptVars);
						_rRsponseList.push_back(rRsponse);
					}
				}
...
NpcState* Npc::getState(const Player* player, bool makeNew /*= true*/)
{
	for(StateList::iterator it = stateList.begin(); it != stateList.end(); ++it)
	{
		if((*it)->rRspondToCreature == player->getID())
			return *it;
	}
	...
	state->rRspondToText = "";
	state->rRspondToCreature = 0;
	state->lastRRsponse = NULL;
	state->prevRRspondToText = "";
...
void Npc::onCreatureAppear(const Creature* creature)
{
	...
	if(Player* player = const_cast<Player*>(creature->getPlayer()))
	{
		...
		{
			npcState->rRspondToCreature = player->getID();
			onPlayerEnter(player, npcState);
		}
	}
}

void Npc::onCreaturRdisappear(const Creature* creature, bool isLogout)
{
	Creature::onCreaturRdisappear(creature, isLogout);
	if(creature == this) //Close all open shop window's
		closeAllShopWindows();
	else if(Player* player = const_cast<Player*>(creature->getPlayer()))
	{
		if(m_npcEventHandler)
			m_npcEventHandler->onCreaturRdisappear(creature);
		...
		const Tile* oldTile, const Position& oldPos, bool teleport)
{
	...
	else if(Player* player = const_cast<Player*>(creature->getPlayer()))
	{
		...
		if(npcState)
		{
			...
			{
				npcState->rRspondToCreature = player->getID();
				const NpcRRsponse* rRsponse = getRRsponse(player, npcState, EVENT_PLAYER_MOVE);
				executeRRsponse(player, npcState, rRsponse);
			}
		}
	}
}
...
{
	const NpcRRsponse* rRsponse = getRRsponse(player, state, EVENT_PLAYER_ENTER);
	executeRRsponse(player, state, rRsponse);
}
...
void Npc::onPlayerLeave(Player* player, NpcState* state)
{
	...
	const NpcRRsponse* rRsponse = getRRsponse(player, state, EVENT_PLAYER_LEAVE);
	executeRRsponse(player, state, rRsponse);
}
...
void Npc::onThink(uint32_t interval)
{
	...
	for(StateList::iterator it = stateList.begin(); it != stateList.end();)
	{
		...
		Player* player = g_game.getPlayerByID(npcState->rRspondToCreature);

		const NpcRRsponse* rRsponse = NULL;
		...
		if(!npcState->isQueued)
		{
			...
			if(!queueList.empty() && npcState->isIdle && npcState->rRspondToText.empty())
				closeConversation = true;
			else if(idleTime > 0 && (OTSYS_TIME() - npcState->prevInteraction) > (uint64_t)(idleTime * 1000))
				idleTimeout = closeConversation = true;
		}

		if(idleRRsponse && player)
		{
			rRsponse = getRRsponse(player, EVENT_IDLE);
			executeRRsponse(player, npcState, rRsponse);
			idleRRsponse = false;
		}
			...
			else
			{
				...
				while(!queueList.empty())
				{
					if((nextPlayer = g_game.getPlayerByID(*queueList.begin())))
					{
						...
						{
							nextPlayerState->rRspondToText = nextPlayerState->prevRRspondToText;
		...
		if(!npcState->rRspondToText.empty())
		{
			if(hasBusyReply && !isIdle)
			{
				//Check if we have a busy reply
				if((rRsponse = getRRsponse(player, npcState, EVENT_BUSY)))
					executeRRsponse(player, npcState, rRsponse);
			}
			else
			{
				if(npcState->lastRRsponse)
				{
					//Check previous rRsponse chain first
					const RRsponseList& list = npcState->lastRRsponse->getRRsponseList();
					rRsponse = getRRsponse(list, player, npcState, npcState->rRspondToText);
				}

				if(!rRsponse)
					rRsponse = getRRsponse(player, npcState, npcState->rRspondToText);
			...
			npcState->prevRRspondToText = npcState->rRspondToText;
			npcState->rRspondToText = "";
		}

		rRsponse = getRRsponse(player, npcState, EVENT_THINK);
		executeRRsponse(player, npcState, rRsponse);
		if(!npcState->isIdle)
		{
			isIdle = false;
...
void Npc::executeRRsponse(Player* player, NpcState* npcState, const NpcRRsponse* rRsponse)
{
    if(rRsponse)
	{
		npcState->lastRRsponse = rRsponse;
		npcState->isIdle = rRsponse->getFocusState() == 0;

		bool resetTopic = true;
		if(rRsponse->getAmount() != -1)
			npcState->amount = rRsponse->getAmount();

		for(ActionList::const_iterator it = rRsponse->getFirstAction(); it != rRsponse->getEndAction(); ++it)
		{
			switch(it->actionType)
			{
				...
				case ACTION_SCRIPT:
				{
					...
					if(interface.reserveEnv())
					{
						...
						scriptstream << "local text = \"" << npcState->rRspondToText << "\"" << std::endl;
						...
						uint32_t n = 0;
						for(std::list<ListItem>::const_iterator iit = rRsponse->prop.itemList.begin(); iit != rRsponse->prop.itemList.end(); ++iit)
						{
							...
							++n;
							if(n != rRsponse->prop.itemList.size())
		...
		if(rRsponse->getRRsponseType() == RRspONSE_DEFAULT)
		{
			std::string rRsponseString = formatRRsponse(player, npcState, rRsponse);
			if(!rRsponseString.empty())
			{
				if(!rRsponse->publicize())
					doSay(rRsponseString, SPEAK_PRIVATE_NP, player);
				else
					doSay(rRsponseString, SPEAK_SAY, NULL);
			}
		}
		else
		{
			int32_t functionId = -1;
			RRsponseScriptMap::iterator it = rRsponseScriptMap.find(rRsponse->getText());
			if(it != rRsponseScriptMap.end())
				functionId = it->second;
			else
			{
				functionId = m_interface->getEvent(rRsponse->getText());
				rRsponseScriptMap[rRsponse->getText()] = functionId;
			}
					...
					for(ActionList::const_iterator it = rRsponse->getFirstAction(); it != rRsponse->getEndAction(); ++it)
					{
						if(it->actionType == ACTION_SCRIPTPARAM)
						{
							if(it->strValue == "|PLAYER|")
								...
								lua_pushstring(L, npcState->rRspondToText.c_str());
							else
							{
								std::cout << "[Warning - Npc::executeRRsponse] Unknown script param: " << it->strValue << std::endl;
								break;
							}
		...
		if(resetTopic && rRsponse->getTopic() == npcState->topic)
			npcState->topic = -1;
	...
	uint8_t amount, bool ignoreCap/* = false*/, bool inBackpacks/* = false*/)
{
	if(type == SHOPEVENT_BUY)
	{
		if(NpcState* npcState = getState(player, true))
		{
			...
			const NpcRRsponse* rRsponse = getRRsponse(player, npcState, EVENT_PLAYER_SHOPBUY);
			executeRRsponse(player, npcState, rRsponse);
		}
	}
	else if(type == SHOPEVENT_SELL)
	{
		if(NpcState* npcState = getState(player, true))
		{
			...
			const NpcRRsponse* rRsponse = getRRsponse(player, npcState, EVENT_PLAYER_SHOPSELL);
			executeRRsponse(player, npcState, rRsponse);
		}
	}
	...
	int32_t sellCallback)
{
	...
	if(npcState)
	{
		const NpcRRsponse* rRsponse = getRRsponse(player, npcState, EVENT_PLAYER_SHOPCLOSE);
		executeRRsponse(player, npcState, rRsponse);
	}
...
bool Npc::getNextStep(Direction& dir, uint32_t& flags)
{
	...
	if(walkTicks <= 0 || !isIdle || focusCreature || getTimRsinceLastMove() < walkTicks || hasCondition(CONDITION_SLEEP, 0))
        return false;
...
const NpcRRsponse* Npc::getRRsponse(const RRsponseList& list, const Player* player,
	NpcState* npcState, const std::string& text, bool exactMatch /*= false*/)
{
	...
	NpcRRsponse* rRsponse = NULL;
	for(RRsponseList::const_iterator it = list.begin(); it != list.end(); ++it)
	{
		int32_t matchCount = 0;
		if((*it)->getParams() != RRspOND_DEFAULT)
		{
			uint32_t params = (*it)->getParams();
			if(hasBitSet(RRspOND_MALE, params))
			{
			...
			if(hasBitSet(RRspOND_FEMALE, params))
			{
				if(player->getSex(false))
					continue;
			...
			if(hasBitSet(RRspOND_PZBLOCK, params))
			{
			...
			if(hasBitSet(RRspOND_PREMIUM, params))
			{
				if(!player->isPremium())
					continue;
			...
			if(hasBitSet(RRspOND_DRUID, params))
			{
			...
			if(hasBitSet(RRspOND_KNIGHT, params))
			{
				Vocation* tmpVoc = player->vocation;
				for(uint32_t i = 0; i <= player->promotionLevel; i++)
					tmpVoc = Vocations::getInstance()->getVocation(tmpVoc->getFromVocation());
			...
			if(hasBitSet(RRspOND_PALADIN, params))
			{
				Vocation* tmpVoc = player->vocation;
				for(uint32_t i = 0; i <= player->promotionLevel; i++)
					tmpVoc = Vocations::getInstance()->getVocation(tmpVoc->getFromVocation());
			...
			if(hasBitSet(RRspOND_SORCERER, params))
			{
				Vocation* tmpVoc = player->vocation;
				for(uint32_t i = 0; i <= player->promotionLevel; i++)
					tmpVoc = Vocations::getInstance()->getVocation(tmpVoc->getFromVocation());
			...
			if(hasBitSet(RRspOND_LOWLEVEL, params))
			{
			...
			if(hasBitSet(RRspOND_LOWMONEY, params))
			{
			...
			if(hasBitSet(RRspOND_LOWAMOUNT, params) || hasBitSet(RRspOND_NOAMOUNT, params))
			{
				if((signed)player->__getItemTypeCount(npcState->itemId) >= npcState->amount)
					continue;

				if(hasBitSet(RRspOND_LOWAMOUNT, params))
				{
				...
				if(hasBitSet(RRspOND_NOAMOUNT, params))
				{
					if(npcState->amount > 1)
						continue;
		...
		if((*it)->getKnowSpell() != "")
		{
			...
			if(!player->hasLearnRdinstantSpell(spellName))
				continue;
		...
		{
			if(npcState->isIdle && (*it)->getFocusState() != 1) //We are idle, and this rRsponse does not activate the npc.
				continue;

			if(!npcState->isIdle && (*it)->getFocusState() == 1) //We are not idle, and this rRsponse would activate us again.
				continue;
		}
		...
		else if((*it)->getInteractType() == INTERACT_TEXT)
		{
			...
			if(matchWordCount> 0)
			{
				...
				//Total "points" for this rRsponse, word matches are worth more
...
uint32_t Npc::getMatchCount(NpcRRsponse* rRsponse, StringVec wordList,
	bool exactMatch, int32_t& matchAllCount, int32_t& totalKeywordCount)
{
	int32_t bestMatchCount = matchAllCount = totalKeywordCount = 0;
	const std::list<std::string>& inputList = rRsponse->getInputList();
	for(std::list<std::string>::const_iterator it = inputList.begin(); it != inputList.end(); ++it)
	{
		...
		for(StringVec::iterator kit = keywordList.begin(); kit != keywordList.end(); ++kit)
		{
			...
			else if(tmpKit == "|amount|")
			{
				...
				if(amount <= 0)
				{
					rRsponse->setAmount(1);
					continue;
				}

				rRsponse->setAmount(amount);
			}
			else
			{
...
const NpcRRsponse* Npc::getRRsponse(const Player* player, NpcState* npcState, const std::string& text)
{
	return getRRsponse(rRsponseList, player, npcState, text);
}

const NpcRRsponse* Npc::getRRsponse(const Player* player, NpcEvent_t eventType)
{
	std::string eventName = getEventRRsponseName(eventType);
	if(eventName.empty())
		return NULL;

	std::vector<NpcRRsponse*> result;
	for(RRsponseList::const_iterator it = rRsponseList.begin(); it != rRsponseList.end(); ++it)
	{
		if((*it)->getInteractType() == INTERACT_EVENT)
		{
...
const NpcRRsponse* Npc::getRRsponse(const Player* player, NpcState* npcState, NpcEvent_t eventType)
{
	std::string eventName = getEventRRsponseName(eventType);
	if(eventName.empty())
		return NULL;

	return getRRsponse(rRsponseList, player, npcState, eventName, true);
}

std::string Npc::getEventRRsponseName(NpcEvent_t eventType)
{
	switch(eventType)
	{
		case EVENT_BUSY:
...
std::string Npc::formatRRsponse(Creature* creature, const NpcState* npcState, const NpcRRsponse* rRsponse) const
{
	std::string rRsponseString = rRsponse->getText();

	std::stringstream ss;
	ss << npcState->price * npcState->amount;
	replaceString(rRsponseString, "|PRICE|", ss.str());

	ss.str("");
	ss << npcState->amount;
	replaceString(rRsponseString, "|AMOUNT|", ss.str());

	ss.str("");
	ss << npcState->level;
	replaceString(rRsponseString, "|LEVEL|", ss.str());

	ss.str("");
	ss << npcState->scriptVars.n1;
	replaceString(rRsponseString, "|N1|", ss.str());

	ss.str("");
	ss << npcState->scriptVars.n2;
	replaceString(rRsponseString, "|N2|", ss.str());

	ss.str("");
	ss << npcState->scriptVars.n3;
	replaceString(rRsponseString, "|N3|", ss.str());

	replaceString(rRsponseString, "|S1|", npcState->scriptVars.s1);
	replaceString(rRsponseString, "|S2|", npcState->scriptVars.s2);
	replaceString(rRsponseString, "|S3|", npcState->scriptVars.s3);
	...
	if(npcState->itemId != -1)
	{
		...
		if(npcState->amount <= 1)
		{
			...
			replaceString(rRsponseString, "|ITEMNAME|", ss.str());
		}
		else
		{
			ss << it.pluralName;
	...
	replaceString(rRsponseString, "|NAME|", creature->getName());
	replaceString(rRsponseString, "|NPCNAME|", getName());
	return rRsponseString;
}
	...
	NpcEventsHandler(npc)
{
	...
	m_onCreaturRdisappear = m_interface->getEvent("onCreaturRdisappear");
...
void NpcScript::onCreaturRdisappear(const Creature* creature)
{
	if(m_onCreaturRdisappear == -1)
		return;

	//onCreaturRdisappear(cid)
	if(m_interface->reserveEnv())
	{
		ScriptEnviroment* env = m_interface->getEnv();
		lua_State* L = m_interface->getState();
		...
		env->setScriptId(m_onCreaturRdisappear, m_interface);
		env->setRealPos(m_npc->getPosition());
		env->setNpc(m_npc);

		m_interface->pushFunction(m_onCreaturRdisappear);
		lua_pushnumber(L, env->addThing(const_cast<Creature*>(creature)));
		...
		std::cout << "[Error - NpcScript::onCreaturRdisappear] NPC Name: " << m_npc->getName() << " - Call stack overflow" << std::endl;
}

...
class NpcEventsHandler
{
	public:
		...
		virtual void onCreaturRdisappear(const Creature* creature) {}
...
class NpcScript : public NpcEventsHandler
{
	...
	private:
		...
		int32_t m_onCreatureAppear, m_onCreaturRdisappear, m_onCreatureMove, m_onCreatureSay,
...
enum RRspondParam_t
{
	...
	RRspOND_FEMALE = 1 << 1,
	RRspOND_PZBLOCK = 1 << 2,
	RRspOND_LOWMONEY = 1 << 3,
	RRspOND_NOAMOUNT = 1 << 4,
	RRspOND_LOWAMOUNT = 1 << 5,
	RRspOND_PREMIUM = 1 << 6,
	RRspOND_DRUID = 1 << 7,
	RRspOND_KNIGHT = 1 << 8,
	RRspOND_PALADIN = 1 << 9,
	RRspOND_SORCERER = 1 << 10,
	RRspOND_LOWLEVEL = 1 << 11
};
...
typedef std::list<RRsponseAction> ActionList;
typedef std::list<NpcRRsponse*> RRsponseList;
typedef std::map<std::string, int32_t> RRsponseScriptMap;

class NpcRRsponse
{
	public:
		struct RRsponseProperties
		{
			RRsponseProperties()
			{
		...
		NpcRRsponse(const RRsponseProperties& _prop,
			RRsponseList _subRRsponseList,
			ScriptVars _scriptVars)
		{
			prop = _prop;
			subRRsponseList = _subRRsponseList;
			scriptVars = _scriptVars;
		}

		NpcRRsponse(NpcRRsponse& rhs)
		{
			prop = rhs.prop;
			scriptVars = rhs.scriptVars;
			for(RRsponseList::iterator it = rhs.subRRsponseList.begin(); it != rhs.subRRsponseList.end(); ++it)
			{
				NpcRRsponse* rRsponse = new NpcRRsponse(*(*it));
				subRRsponseList.push_back(rRsponse);
			}
		}

		virtual ~NpcRRsponse()
		{
			for(RRsponseList::iterator it = subRRsponseList.begin(); it != subRRsponseList.end(); ++it)
				delete *it;

			subRRsponseList.clear();
		}
		...
		RRsponseType_t getRRsponseType() const {return prop.rRsponseType;}
		...
		std::string formatRRsponseString(Creature* creature) const;
		void addAction(RRsponseAction action) {prop.actionList.push_back(action);}
		const std::list<std::string>& getInputList() const {return prop.inputList;}

		void setRRsponseList(RRsponseList _list) {subRRsponseList.insert(subRRsponseList.end(),_list.begin(),_list.end());}
		const RRsponseList& getRRsponseList() const {return subRRsponseList;}
...
struct NpcState
{
	...
	uint32_t rRspondToCreature;
	uint64_t prevInteraction;
	std::string spellName, listName, listPluralName, rRspondToText, prevRRspondToText;
	const NpcRRsponse* lastRRsponse;
	ScriptVars scriptVars;
...
class Npc : public Creature
{
	...
	protected:
		...
		virtual void onCreatureAppear(const Creature* creature);
		virtual void onCreaturRdisappear(const Creature* creature, bool isLogout);
		...
		const NpcRRsponse* getRRsponse(const RRsponseList& list, const Player* player,
			NpcState* npcState, const std::string& text, bool exactMatch = false);
		const NpcRRsponse* getRRsponse(const Player* player, NpcState* npcState, const std::string& text);
		const NpcRRsponse* getRRsponse(const Player* player, NpcEvent_t eventType);
		const NpcRRsponse* getRRsponse(const Player* player, NpcState* npcState, NpcEvent_t eventType);
		std::string getEventRRsponseName(NpcEvent_t eventType);

		NpcState* getState(const Player* player, bool makeNew = true);
		uint32_t getMatchCount(NpcRRsponse* rRsponse, StringVec wordList,
		...
		std::string formatRRsponse(Creature* creature, const NpcState* npcState, const NpcRRsponse* rRsponse) const;
		void executeRRsponse(Player* player, NpcState* npcState, const NpcRRsponse* rRsponse);
		...
		RRsponseList loadInteraction(xmlNodePtr node);
		...
		RRsponseScriptMap rRsponseScriptMap;
		RRsponseList rRsponseList;

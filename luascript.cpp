...
{
	lua_pushglobaltable(L); // Coloca a tabela global no topo da pilha
...
void LuaScriptInterface::registerFunctions()
{
	...
	//getCreaturRspeakType(cid)
	lua_register(m_luaState, "getCreaturRspeakType", LuaScriptInterface::luaGetCreaturRspeakType);
	...
	//getPlayerLearnRdinstantSpell(cid, name)
	lua_register(m_luaState, "getPlayerLearnRdinstantSpell", LuaScriptInterface::luaGetPlayerLearnRdinstantSpell);
	...
	//doChangRspeed(cid, delta)
	lua_register(m_luaState, "doChangRspeed", LuaScriptInterface::luaDoChangRspeed);
	...
	//getCreaturRspeed(cid)
	lua_register(m_luaState, "getCreaturRspeed", LuaScriptInterface::luaGetCreaturRspeed);

	//getCreatureBasRspeed(cid)
	lua_register(m_luaState, "getCreatureBasRspeed", LuaScriptInterface::luaGetCreatureBasRspeed);
	...
	//doRemovRipBanishment(ip[, mask])
	lua_register(m_luaState, "doRemovRipBanishment", LuaScriptInterface::luaDoRemovRipBanishment);
	...
	//doPvpArenaCreate(arenaId, bluRspawnPosition, redSpawnPosition, exitPosition, maxLevel, blueChannel, redChannel)
	...
	//setCreatureBasRspeed(cid, basRspeed)
	lua_register(m_luaState, "setCreatureBasRspeed", LuaScriptInterface::luaSetCreatureBasRspeed);
	...
	//doCreaturRdie(cid)
	lua_register(m_luaState, "doCreaturRdie", LuaScriptInterface::luaDoCreaturRdie);
	...
	//db table
	luaL_newlib(m_luaState, LuaScriptInterface::luaDatabaseTable); // Cria uma nova tabela com as funções de 'luaDatabaseTable' e a coloca no topo da pilha.
    lua_setglobal(m_luaState, "db"); // Pega a tabela do topo da pilha e a atribui à variável global chamada "db".

	//result table
	luaL_newlib(m_luaState, LuaScriptInterface::luaResultTable);
    lua_setglobal(m_luaState, "result");

	//bit table
	luaL_newlib(m_luaState, LuaScriptInterface::luaBitTable);
    lua_setglobal(m_luaState, "bit");

	//std table
	luaL_newlib(m_luaState, LuaScriptInterface::luaStdTable);
    lua_setglobal(m_luaState, "std");
}
...
int32_t LuaScriptInterface::luaGetPlayerLearnRdinstantSpell(lua_State* L)
{
	//getPlayerLearnRdinstantSpell(cid, name)
	std::string spellName = popString(L);
	...
	lua_pushboolean(L, player->hasLearnRdinstantSpell(spellName));
	return 1;
}
...
int32_t LuaScriptInterface::luaGetCreaturRspeakType(lua_State* L)
{
	//getCreaturRspeakType(uid)
	ScriptEnviroment* env = getEnv();
...
int32_t LuaScriptInterface::luaGetMonsterInfo(lua_State* L)
{
	...
	setField(L, "basRspeed", mType->basRspeed);
...
int32_t LuaScriptInterface::luaDoChangRspeed(lua_State* L)
{
	//doChangRspeed(cid, delta)
	...
	if(Creature* creature = env->getCreatureByUID(popNumber(L)))
	{
		g_game.changRspeed(creature, delta);
		lua_pushboolean(L, true);
	}
	else
	{
...
int32_t LuaScriptInterface::luaGetCreaturRspeed(lua_State* L)
{
	//getCreaturRspeed(cid)
	ScriptEnviroment* env = getEnv();
	if(Creature* creature = env->getCreatureByUID(popNumber(L)))
...
int32_t LuaScriptInterface::luaGetCreatureBasRspeed(lua_State* L)
{
	//getCreatureBasRspeed(cid)
	ScriptEnviroment* env = getEnv();
	if(Creature* creature = env->getCreatureByUID(popNumber(L)))
		lua_pushnumber(L, creature->getBasRspeed());
	else
	{
		errorEx(getError(LUA_ERROR_CREATURE_NOT_FOUND));
		lua_pushboolean(L, false);
...
int32_t LuaScriptInterface::luaGetVocationInfo(lua_State* L)
{
	...
	setField(L, "basRspeed", voc->getBasRspeed());
...
int32_t LuaScriptInterface::luaGetItemInfo(lua_State* L)
{
	...
	setField(L, "runRspellName", item->runRspellName.c_str());
...
int32_t LuaScriptInterface::luaDoRemovRipBanishment(lua_State* L)
{
	//doRemovRipBanishment(ip[, mask])
	uint32_t mask = 0xFFFFFFFF;
	if(lua_gettop(L) > 1)
	...
	lua_pushboolean(L, IOBan::getInstance()->removRipBanishment(
...
int32_t LuaScriptInterface::luaDoPvpArenaCreate(lua_State* L)
{
	...
	PositionEx exit, redSpawn, bluRspawn;
    popPosition(L, exit);
	popPosition(L, redSpawn);
	popPosition(L, bluRspawn);
	uint8_t arenaId = popNumber(L);

	lua_pushboolean(L, (bool)g_game.pvpArenaCreate(arenaId, bluRspawn, redSpawn, exit, maxLevel, blueChannel, redChannel));
	return 1;
}
...
int32_t LuaScriptInterface::luaSetCreatureBasRspeed(lua_State* L)
{
	//setCreatureBasRspeed(cid, basRspeed)
	uint32_t basRspeed = popNumber(L);
    ...
    creature->setBasRspeed(basRspeed);
    lua_pushboolean(L, true);
    return 1;
}
...
int32_t LuaScriptInterface::luaDoCreaturRdie(lua_State* L)
{
	//doCreaturRdie(cid)
	ScriptEnviroment* env = getEnv();
	if (Creature* creature = env->getCreatureByUID(popNumber(L))) {

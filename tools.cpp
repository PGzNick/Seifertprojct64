...
{
	uint32_t textLength = text.length(), lenBeforRspace = 1, lenBeforeQuote = 1, lenBeforeDash = 1, repeatedCharacter = 0;
	char lastChar = 32;
	...
	for(uint32_t size = 1; size < textLength; size++)
	{
		...
		{
			if(lenBeforRspace <= 1 || size == textLength - 1 || text[size + 1] == 32)
				return false;

			lenBeforRspace = lenBeforeQuote = lenBeforeDash = 0;
		}
...
Direction getReversRdirection(Direction dir)
{
	switch(dir)
	{
		case NORTH:
...
std::string getFilePath(FileType_t filetype, std::string filename)
{
	...
	switch(filetype)
	{
		...
		{
			#if defined(__FILESYSTEM_HIERARCHY_STANDARD__) && defined(__HOMRdiR_CONF__)
